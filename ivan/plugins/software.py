...
def parse_22869(soft_dict):
    software_data = db_query("select output, asset_ip from vulns where plugin_id='22869'")
...
def parse_20811(soft_dict):
    software_data = db_query("select output, asset_ip from vulns where plugin_id='20811'")
    for data in software_data:
        asset_uuid = data[1]
        for pkg in data:
...
def parse_83991(soft_dict):
    software_data = db_query("select output, asset_ip from vulns where plugin_id='83991'")
...
def display_stats():
    try:
        ...
        asset_total = db_query("select count(distinct asset_ip) from vulns;")[0][0]
        assets_with_data = db_query("select count(distinct asset_ip) from vulns "
                                    "where plugin_id ='22869' or plugin_id ='20811';")[0][0]
        assets_without_data = db_query("select distinct asset_ip from vulns where asset_ip not in "
                                       "(select distinct asset_ip from vulns where plugin_id ='22869' "
                                       "or plugin_id ='20811')")
        ...
        click.echo("\nYou need to run 'ivan software generate' to populate the software table.\n")
